...
title_name <- 'q25_Sx_48,52_RAM repression' # make title_name for plot and saving
...
# create a subset of data for presenting : ONLY 51 and 76
processed_data <- filter(absolute_dat,
                           str_detect(assay_variable, '51|76'), # Select only 76
                         !str_detect(Target_name, '16s')) %>%  # remove 16S
  
  mutate(across(assay_variable, ~ str_replace_all(.x, assay_var_translator))) %>%  # assign labels for readability
...
label_data <- 
  select(processed_data, assay_variable, Sample_category, 
         Target_name, mean_Copies.per.ul.template) %>%
  unique() %>%  # select unique values
  
  # add a column for labels to cat-RNA (3,4)
  mutate(label_categories = c(NA, 'Repressed', NA, 'Constitutive',
                              rep(NA, 3), 'Barcoded rRNA') 
                              ) 
...
{ggplot(processed_data, 
        aes(y = assay_variable, x = Copies.per.ul.template, 
...
write_csv(processed_data, str_c('excel files/paper_data/', title_name, '.csv')) # save data as csv
