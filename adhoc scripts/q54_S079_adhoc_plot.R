...
copies_wider <-
  ...
  select(Target_name, assay_var.horz_label, Copies.per.ul.template, biological_replicates) %>%  # select only required columns
...
plt.positivesonly_correlation <- 
  {ggplot(
    ...
    filter(copies_wider, !str_detect(organism, 'NTC|(-)|552')), 
