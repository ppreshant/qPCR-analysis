...
# map2(list_rawqpcr, read_these_sheets, ~print(plot_biological_replicates(.x, title_text = .y)))
...
presentable_vol_qpcr <- vol_qpcr %>% rename('Facility' = `FACILITY NAME`) %>% 
  ...
  mutate_at('Facility', ~if_else(. == assay_variable, str_c(`Sample Name`, '-', assay_variable, '.', biological_replicates), .)) %>% 
  ...
  select(Facility, WWTP, WW_vol, Target, CT, `Copy #`, Recovered, `Recovery fraction`) %>% 
