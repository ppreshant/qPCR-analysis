...
# Calibration wrapper ----
#' wrapper function for vectorized calibration
load_procesessed_data_and_calibrate <- function(flnm, make_plot = FALSE)
{
  # Load data ----
  ...
  # Get std curve ----
  if(!exists('std_par')) # ignore this if parameters are already loaded
  {
    std_to_retrieve <<- default_std.to.retrieve # load as global variable
  ...
  return(absolute_dat) # return absolute data from the function
