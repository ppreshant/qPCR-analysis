...
# Enter file name
flnm <- 'q-S023b GFP 18-2-20'  # set the filename
...
# filtering a single well data = A1

# plotting normal
plt1 <- combdat %>% filter(`Well Position` == 'A1') %>%  ggplot(aes(Cycle, signal, colour = channel)) + geom_point() + geom_line()

# interactive plot
combdat %>% filter(`Well Position` == 'A1') %>%  plot_ly(x = ~Cycle, y = ~signal, color = ~channel, type = 'scatter', mode = 'lines+markers')
